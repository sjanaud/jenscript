...
function createViewStockTimePicker(container, width, height) {
	...
	//nodeView1.setAttribute('style','padding-right : 5px;');
		...
		paintPlugin : function(g2d, part) {
			if (part === JenScript.ViewPart.Device) {
				...
				if(minPixel>0){
					...
					g2d.insertSVG(svgRectLeft.fill(JenScript.RosePalette.EMERALD).fillOpacity(0.5).toSVG());
				}
				...
				if(maxPixel-minPixel>0){
					...
					g2d.insertSVG(svgRectView.fill(JenScript.RosePalette.MANDARIN).fillOpacity(0.4).toSVG());
				}
				...
				if(dw-maxPixel>0){
					...
					g2d.insertSVG(svgRectRight.fill(JenScript.RosePalette.EMERALD).fillOpacity(0.5).toSVG());
				}
