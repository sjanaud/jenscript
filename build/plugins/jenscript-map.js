...
(function(){
	...
	JenScript.Queue = function(data, compare) {
		if (!(this instanceof JenScript.Queue)) return new JenScript.Queue(data, compare);
	    this.data = data || [];
	...
	JenScript.Model.addMethods(JenScript.Queue,{
	...
	JenScript.PolygonLabel = function(polygon, precision, debug) {
	    this.init(polygon, precision, debug)
	};
	...
	JenScript.Model.addMethods(JenScript.PolygonLabel,{
		init : function(polygon, precision, debug){
		    ...
		    var cellQueue = new JenScript.Queue(null, compareMax);
